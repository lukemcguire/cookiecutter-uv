...
def test_using_pytest(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        with run_within_dir(str(result.project_path)):
            ...
            assert subprocess.check_call(shlex.split("just test")) == 0  # noqa: S603  # controlled test command
...
def test_devcontainer(cookies, tmp_path):
    ...
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert pathlib.Path(f"{result.project_path}/.devcontainer/devcontainer.json").is_file()
        assert pathlib.Path(f"{result.project_path}/.devcontainer/postCreateCommand.sh").is_file()
...
def test_not_devcontainer(cookies, tmp_path):
    ...
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert not pathlib.Path(f"{result.project_path}/.devcontainer/devcontainer.json").is_file()
        assert not pathlib.Path(f"{result.project_path}/.devcontainer/postCreateCommand.sh").is_file()
...
def test_mkdocs(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert pathlib.Path(f"{result.project_path}/docs").is_dir()
...
def test_dockerfile(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert pathlib.Path(f"{result.project_path}/Dockerfile").is_file()
...
def test_not_dockerfile(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert not pathlib.Path(f"{result.project_path}/Dockerfile").is_file()
...
def test_codecov(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert is_valid_yaml(result.project_path / ".github" / "workflows" / "main.yml")
        assert pathlib.Path(f"{result.project_path}/codecov.yaml").is_file()
        assert pathlib.Path(f"{result.project_path}/.github/workflows/validate-codecov-config.yml").is_file()
...
def test_not_codecov(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert is_valid_yaml(result.project_path / ".github" / "workflows" / "main.yml")
        assert not pathlib.Path(f"{result.project_path}/codecov.yaml").is_file()
        assert not pathlib.Path(f"{result.project_path}/.github/workflows/validate-codecov-config.yml").is_file()
...
def test_remove_release_workflow(cookies, tmp_path):
    with run_within_dir(tmp_path):
        result = cookies.bake(extra_context={"publish_to_pypi": "n", "mkdocs": "y"})
        assert result.exit_code == 0
        assert pathlib.Path(f"{result.project_path}/.github/workflows/on-release-main.yml").is_file()

        result = cookies.bake(extra_context={"publish_to_pypi": "n", "mkdocs": "n"})
        assert result.exit_code == 0
        assert not pathlib.Path(f"{result.project_path}/.github/workflows/on-release-main.yml").is_file()
...
def test_license_mit(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert pathlib.Path(f"{result.project_path}/LICENSE").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_BSD").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_ISC").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_APACHE").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_GPL").is_file()
        with pathlib.Path(f"{result.project_path}/LICENSE").open(encoding="utf8") as licfile:
...
def test_license_bsd(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert pathlib.Path(f"{result.project_path}/LICENSE").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_MIT").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_ISC").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_APACHE").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_GPL").is_file()
        with pathlib.Path(f"{result.project_path}/LICENSE").open(encoding="utf8") as licfile:
...
def test_license_no_license(cookies, tmp_path):
    with run_within_dir(tmp_path):
        ...
        assert result.exit_code == 0
        assert not pathlib.Path(f"{result.project_path}/LICENSE").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_MIT").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_BSD").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_ISC").is_file()
        assert not pathlib.Path(f"{result.project_path}/LICENSE_APACHE").is_file()
